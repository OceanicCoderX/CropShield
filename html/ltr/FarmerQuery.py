...
query = """Select * from farmerquery"""
mycursor.execute(query)
myresult = mycursor.fetchall()

tr_html = ''
for x in myresult:
    fqid, fid, fname, cname, pname, sdetails, simg, qstatus, date = x
    ...
    <tr class="unread">
        ...
        <td class="max-texts"><a class="link" href="QueryBg.py?qid={fqid}">
            <span class="blue-grey-text text-darken-4">{(sdetails[:80] + '...') if sdetails and len(sdetails)>80 else (sdetails or '')}</a>
        </td>
        <td class="clip"><b>{qstatus}</b></td>
        <td class="time"><a href="QueryBg.py?qid={fqid}"><i class="fa-solid fa-reply"></i> read</a></td>
    </tr>
'''
