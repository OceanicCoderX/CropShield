...
query = "SELECT p.PId, pc.Name, d.Name, p.Name, p.Image, p.Detail, p.Usege FROM product p JOIN productcategory pc ON p.PCId = pc.PCId JOIN diseases d ON p.DId = d.DId"
...
for x in myresult:
    tr_html += f'''
    <tr>
        <td><a href="ProductDelete.py?cid={x[0]}"><i class="fa-solid fa-trash"></i></a>
        <a href="ProductEdit.py?cid={x[0]}"><i class="fa-solid fa-pen-to-square"></i></a></td>
        <td>{x[0]}</td>
        <td>{x[1]}</td>
        <td>{x[2]}</td>
        <td>{x[3]}</td>
        <td><img src="assets/Product/{x[0]}/{x[4]}" style="width: 100px; height: 100px;"></td>
        <td>{x[5]}</td>
        <td>{x[6]}</td>
    </tr>
    '''
